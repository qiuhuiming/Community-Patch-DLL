...
def _apply_xml(xml_path: Path, dictionary: dict[str, str]) -> bool:
    ...
    if zh_match:
        ...
        existing = _existing_tags_in_block(zh_body)
        ...
        for kind, tag, en_text in en_entries:
            if tag in existing:
                continue
            additions.append(_render_entry(kind, tag, dictionary.get(tag, en_text), indent))

        if not additions:
            return False

        insert_at = zh_match.end(1)
        updated = raw[:insert_at] + "".join(additions) + raw[insert_at:]
    else:
